...
parser.add_argument('--azure-china', {
    help: 'Azure only. Enables Azure China mode.',
    action: 'store_true'
});
...
if (config.credentials.aws.credential_file && (!settings.cloud || (settings.cloud == 'aws'))) {
    ...
    cloudConfig.Govcloud = config.credentials.azure.govcloud;
    cloudConfig.China = settings['azure-china'] || config.credentials.azure.china;
    ...
    cloudConfig = {
        ...
        China: settings['azure-china'] || config.credentials.azure.china,
...
// Sync Azure China flag from cloudConfig to settings for plugin access
if (settings.cloud === 'azure' && cloudConfig.China) {
    settings.azure_china = true;
}
