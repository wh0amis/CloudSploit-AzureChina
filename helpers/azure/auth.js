...
var locations_china = require(__dirname + '/locations_china.js');
...
module.exports = {
    login: function(azureConfig, callback) {
        ...
        let credential;
        if (azureConfig.China) {
            // Azure China uses a different authority endpoint
        ...
        } else if (azureConfig.Govcloud) {
            // Azure US Government uses a different authority endpoint
            credential = new ClientSecretCredential(
                azureConfig.DirectoryID,
                azureConfig.ApplicationID,
                azureConfig.KeyValue,
        ...
        if (azureConfig.China) {
            const armScope = 'https://management.chinacloudapi.cn/.default';
            const graphScope = 'https://graph.chinacloudapi.cn/.default';
            const vaultScope = 'https://vault.azure.cn/.default';
    ...
    call: function(params, callback) {
        ...
        if (params.china) {
            params.url = params.url.replace('management.azure.com', 'management.chinacloudapi.cn');
            params.url = params.url.replace('graph.microsoft.com', 'graph.chinacloudapi.cn');
            params.url = params.url.replace('vault.azure.net', 'vault.azure.cn');
        } else if (params.govcloud) {
            params.url = params.url.replace('management.azure.com', 'management.usgovcloudapi.net');
        }
    ...
    addChinaLocations: function(obj, service, collection, err, data, skip_locations) {
        if (!service || !locations_china[service]) return;
        locations_china[service].forEach(function(location) {
            if (skip_locations.includes(location)) return;
            collection[location.toLowerCase()] = {};
            if (err) {
                collection[location.toLowerCase()].err = err;
