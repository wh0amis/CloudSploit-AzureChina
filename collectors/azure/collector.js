        ...
        let processCall = function(obj, cb, localData) {
            ...
            if (fargateFlag) {
                ...
                async.retry({
                    ...
                    // Apply China/Govcloud endpoint replacements for display purposes
                    let displayUrl = localUrl;
                    if (AzureConfig.China) {
                        displayUrl = displayUrl.replace('management.azure.com', 'management.chinacloudapi.cn');
                        displayUrl = displayUrl.replace('graph.microsoft.com', 'graph.chinacloudapi.cn');
                        displayUrl = displayUrl.replace('vault.azure.net', 'vault.azure.cn');
                    } else if (AzureConfig.Govcloud) {
                        displayUrl = displayUrl.replace('management.azure.com', 'management.usgovcloudapi.net');
                    }

                    var rateLimit = obj.rateLimit && obj.rateLimit == 3000? 6000: obj.rateLimit;
                    if (rateLimit) {
                        setTimeout(function() {
                            console.log(`Fargate collector rate limited: url: ${displayUrl}`);
            ...
            function(cb) {
                function processTopCall(collectionObj, service, subCallObj, subCallCb) {
                    processCall(subCallObj, function(processCallErr, processCallData) {
                        if (AzureConfig.China) helpers.addChinaLocations(subCallObj, service, collectionObj, processCallErr, processCallData , skip_locations);
                        else if (AzureConfig.Govcloud) helpers.addGovLocations(subCallObj, service, collectionObj, processCallErr, processCallData , skip_locations);
